...
const random_activity = require("./random_activity.json");
const queue = new Map();
const client = new Discord.Client();

client.once('ready', () => {
    console.log('Ready');
    const status = random_activity.activity
    setInterval(function(){
        let activity = status[Math.floor(Math.random()*status.length)];
        client.user.setActivity(activity, {type: "PLAYING"}).catch(console.error)
    }, 10000);
});
